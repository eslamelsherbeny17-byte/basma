...
export default function CategoriesManagement() {
  ...
  return (
    <div className='space-y-4 sm:space-y-6 p-3 sm:p-4 md:p-6' dir="rtl">
      ...
      <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 bg-white p-3 sm:p-4 rounded-xl border shadow-sm'>
        <div className='w-full sm:w-auto'>
          <h1 className='text-xl sm:text-2xl font-bold text-gray-800'>إدارة الفئات</h1>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-6'>
        {loading ? (
          ...
          categories.map((category) => (
            <Card 
              key={category._id} 
              className='group overflow-hidden hover:shadow-xl transition-all duration-300 border-border/60 flex flex-col h-full bg-white active:scale-95'
            >
              ...
              <div className='relative w-full aspect-square overflow-hidden bg-white'>
                ...
                {category.image ? (
                  ...
                  <div className='flex items-center justify-center h-full bg-gradient-to-br from-gray-50 to-gray-100'>
                    <FolderTree className='h-10 w-10 sm:h-12 sm:w-12 md:h-16 md:w-16 text-gray-300' />
                  </div>
                )}
              </div>

              {/* Content Section - Mobile Optimized */}
              <div className='p-2.5 sm:p-3 md:p-4 flex-1 flex flex-col justify-between border-t bg-gradient-to-b from-white to-gray-50/50'>
                <div className='mb-2 sm:mb-3'>
                  <h3 className='font-bold text-sm sm:text-base md:text-lg text-gray-800 truncate mb-1 sm:mb-2'>
                    {category.name}
                  </h3>
      ...
      {/* Image Preview Lightbox - Mobile Optimized */}
      <Dialog open={imagePreviewOpen} onOpenChange={setImagePreviewOpen}>
        <DialogContent className="w-screen h-screen max-w-none sm:w-[95vw] sm:h-auto sm:max-w-4xl p-0 bg-transparent border-none shadow-none">
          <div className="relative bg-white rounded-none sm:rounded-2xl overflow-hidden shadow-2xl h-full sm:h-auto">
            ...
            {previewImage && (
              <div className="relative w-full h-full sm:h-[70vh] bg-white flex items-center justify-center">
...
function CategoryForm({ category, onSuccess }: { category?: any, onSuccess: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className='space-y-3 sm:space-y-4 pt-1'>
      ...
      <div 
        className="flex items-center justify-between p-2.5 sm:p-3 border-2 rounded-lg bg-gradient-to-br from-gray-50 to-white cursor-pointer hover:border-primary/30 active:border-primary/50 transition-all"
        onClick={() => setShowOnHomePage(!showOnHomePage)}
      >
        <div className="flex flex-col gap-0.5 text-right flex-1">
          <Label className="font-bold cursor-pointer text-xs sm:text-sm">
            الظهور في الصفحة الرئيسية
          </Label>
          <span className="text-[9px] sm:text-[10px] text-muted-foreground leading-tight">
            عرض الفئة في الواجهة الرئيسية
          </span>
        </div>
      ...
      <div className='space-y-1.5'>
        ...
        <div className='border-2 border-dashed rounded-lg p-3 sm:p-4 text-center bg-gradient-to-br from-gray-50 to-white hover:border-primary/30 active:border-primary/50 transition-all'>
          {imagePreview ? (
            <div className='relative w-full aspect-square max-h-40 sm:max-h-48 mb-2 sm:mb-3 rounded-lg overflow-hidden bg-white border border-gray-100'>
            ...
            <div className='py-4 sm:py-5'>
              <FolderTree className="h-10 w-10 sm:h-12 sm:w-12 mx-auto text-gray-300 mb-1.5 sm:mb-2" />
